            ...
            <template #title>
                <div class="card-header">
                    ...
                    <div class="header-controls">
                        ...
                        <a-select v-model:value="selectedDays" style="width: 120px" @change="handleDaysChange">
                        ...
                        <a-tab-pane key="activity" tab="活动数据">
                            <div class="summary-grid">
                                ...
                                <a-card class="summary-item">
                                    ...
                                    <div class="value">{{ healthSummary.activity.distance_km ?
                                        `${healthSummary.activity.distance_km} km` : '暂无数据'
                                        }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>消耗卡路里</template>
                                    <div class="value">{{ healthSummary.activity.calories_burned ?
                                        `${healthSummary.activity.calories_burned} kcal`
                                        : '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>活动时间</template>
                                    <div class="value">{{ healthSummary.activity.active_minutes ?
                                        `${healthSummary.activity.active_minutes} 分钟` :
                                        '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                        ...
                        <a-tab-pane key="sleep" tab="睡眠数据">
                            <div class="summary-grid">
                                <a-card class="summary-item">
                                    ...
                                    <div class="value">{{ healthSummary.sleep.total_sleep_hours ?
                                        `${healthSummary.sleep.total_sleep_hours}
                                        小时` : '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>深度睡眠</template>
                                    <div class="value">{{ healthSummary.sleep.deep_sleep_hours ?
                                        `${healthSummary.sleep.deep_sleep_hours}
                                        小时` : '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>浅度睡眠</template>
                                    <div class="value">{{ healthSummary.sleep.light_sleep_hours ?
                                        `${healthSummary.sleep.light_sleep_hours}
                                        小时` : '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>REM睡眠</template>
                                    <div class="value">{{ healthSummary.sleep.rem_sleep_hours ?
                                        `${healthSummary.sleep.rem_sleep_hours} 小时`
                                        : '暂无数据' }}</div>
                                </a-card>
                                <a-card class="summary-item">
                                    <template #title>睡眠效率</template>
                                    <div class="value">{{ healthSummary.sleep.sleep_efficiency ?
                                        `${healthSummary.sleep.sleep_efficiency}%`
                                        : '暂无数据' }}</div>
                                </a-card>
                            </div>
                        </a-tab-pane>
                    ...
                    <div class="additional-info">
                        <a-card title="其他信息" class="mt-4">
                            ...
                            <div class="key-insights"
                                v-if="healthSummary.insights && healthSummary.insights.length > 0">
                                <h3>关键洞察</h3>
                                <ul>

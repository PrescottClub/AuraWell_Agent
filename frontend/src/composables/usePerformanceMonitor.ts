...
export function usePerformanceMonitor() {
    ...
    Object.entries(weights).forEach(([metric, weight]) => {
      ...
      if (value !== undefined && threshold && typeof value === 'number') {
        ...
        if (value > threshold.poor) {
          ...
          metricScore =
            50 *
            (1 - (value - threshold.good) / (threshold.poor - threshold.good));
        }
  ...
  const generateRecommendations = (): string[] => {
    ...
    if (
      ...
      metrics.value.loadTime > thresholds.loadTime.poor
    ) {
      recs.push('页面加载时间过长，建议优化资源加载和代码分割');
    }

    if (
      metrics.value.firstContentfulPaint &&
      metrics.value.firstContentfulPaint > thresholds.firstContentfulPaint.poor
    ) {
      recs.push('首次内容绘制时间过长，建议优化关键渲染路径');
    }

    if (
      metrics.value.largestContentfulPaint &&
      metrics.value.largestContentfulPaint >
        thresholds.largestContentfulPaint.poor
    ) {
    ...
    if (
      metrics.value.firstInputDelay &&
      metrics.value.firstInputDelay > thresholds.firstInputDelay.poor
    ) {
    ...
    if (
      ...
      metrics.value.cumulativeLayoutShift >
        thresholds.cumulativeLayoutShift.poor
    ) {
    ...
    if (
      ...
      metrics.value.memoryUsage > thresholds.memoryUsage.poor
    ) {
      recs.push('内存使用过高，建议检查内存泄漏和优化数据结构');
    }

    if (
      metrics.value.frameRate &&
      metrics.value.frameRate < thresholds.frameRate.poor
    ) {

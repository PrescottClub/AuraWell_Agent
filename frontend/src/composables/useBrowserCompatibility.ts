...
export function useBrowserCompatibility() {
  ...
  const detectCSSFeatures = (): Partial<FeatureSupport> => {
    ...
    features.cssContainerQueries = CSS.supports(
      'container-type',
      'inline-size'
    );
  ...
  const generateWarnings = (
    ...
    if (!browser.supported) {
      warns.push(
        `您的浏览器版本 ${browser.name} ${browser.version} 可能不完全支持所有功能`
      );
    }
    ...
    if (!features.intersectionObserver) {
      warns.push(
        '您的浏览器不支持 Intersection Observer，滚动动画可能无法正常工作'
      );
    }

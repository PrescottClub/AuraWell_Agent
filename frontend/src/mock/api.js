...
export class HealthPlanAPI extends MockAPI {
  ...
  async getUserHealthPlans(params = {}) {
    ...
    let plans = mockData.healthPlans.filter(
      plan => plan.user_id === mockData.currentUser.user_id
    );
  ...
  async updateHealthPlan(planId, updateData) {
    ...
    const planIndex = mockData.healthPlans.findIndex(
    ...
    return this.createResponse(
      mockData.healthPlans[planIndex],
      true,
      '健康计划更新成功'
    );
  }
...
export class ChatAPI extends MockAPI {
  ...
  async getChatSessions(params = {}) {
    ...
    const sessions = mockData.chatSessions.filter(
      session => session.user_id === mockData.currentUser.user_id
    );
  ...
  async getChatSession(sessionId) {
    ...
    const session = mockUtils.findById(
...
export class FamilyAPI extends MockAPI {
  ...
  async getUserFamilies() {
    ...
    const families = mockData.families.filter(family => {
      return mockData.familyMembers.some(
        member =>
  ...
  async getFamilyMembers(familyId) {
    ...
    const members = mockData.familyMembers.filter(
  ...
  async getFamilyPermissions(familyId) {
    ...
    const member = mockData.familyMembers.find(
      m =>
  ...
  async switchActiveMember(data) {
    ...
    const member = mockUtils.findById(
      mockData.familyMembers,
      member_id,
      'member_id'
    );
  ...
  async getFamilyHealthReport(familyId) {
    ...
    const mockReport = {
      summary: {
        total_steps: 25000,
        avg_sleep_hours: 7.5,
        active_members: mockData.familyMembers.filter(
  ...
  async getFamilyChallenges(familyId) {
    ...
    const challenges = mockData.familyChallenges.filter(
  ...
  async createFamilyChallenge(familyId, challengeData) {
    ...
    const newChallenge = {
      ...
      end_date: new Date(
        Date.now() + challengeData.duration_days * 24 * 60 * 60 * 1000
      ).toISOString(),
  ...
  async getHealthAlerts(familyId) {
    ...
    const alerts = mockData.healthAlerts.filter(
      alert => alert.family_id === familyId
    );
...
export class HealthReportAPI extends MockAPI {
  ...
  async getHealthReports(params = {}) {
    ...
    let reports = mockData.healthReports.filter(
      report => report.user_id === mockData.currentUser.user_id
    );
  ...
  async getHealthReport(reportId) {
    ...
    const report = mockUtils.findById(
      mockData.healthReports,
      reportId,
      'report_id'
    );
    if (!report) {

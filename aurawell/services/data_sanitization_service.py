...
from ..models.api_models import (
    FamilyRole, DataAccessLevel, MemberDataContext, 
    DataSanitizationRule, SanitizedUserData, DataPrivacySettings
)
...
class DataSanitizationService:
    ...
    async def sanitize_user_data(
        ...
        data_context: MemberDataContext
    ) -> SanitizedUserData:
    ...
    def _apply_sanitization_rule(
        ...
        if rule.sanitization_type == "mask":
            ...
            data[rule.field_name] = self._anonymize_field_value(
                data[rule.field_name]
            )
    ...
    def _get_allowed_fields(
        ...
        if access_level == DataAccessLevel.FULL:
            ...
            return base_fields + [
                "age", "gender", "activity_level", "basic_health_metrics",
                "goals_summary", "achievements_summary"
            ]
        else:  # BASIC
    ...
    def _check_data_type_permissions(
        ...
        if role == FamilyRole.OWNER:
            ...
            allowed_types = [
                "basic_health_info", "activity_summary", "goals_summary"
            ]
            return all(dt in allowed_types for dt in data_types)
        else:
            return False
    
    def _extract_basic_health_info(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _extract_activity_summary(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:

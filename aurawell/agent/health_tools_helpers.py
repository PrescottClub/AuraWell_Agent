...
def _personalize_exercise_plan(base_plan: dict, user_profile, current_activity_level: str) -> dict:
    """个性化调整运动计划"""
...
def _calculate_expected_results(goal_type: str, duration_weeks: int, fitness_level: str, user_profile) -> dict:
    """计算预期运动效果"""
    results = {}
...
def _get_exercise_library(goal_type: str, fitness_level: str) -> dict:
    ...
    strength_exercises = {
        ...
        "advanced": ["复合动作", "爆发力训练", "功能性训练", "专项训练", "高级核心训练"]
    }
...
def _generate_executive_summary(user_data: dict, analysis_period: dict) -> str:
    ...
    # 计算关键指标
    avg_steps = sum(a.get("steps", 0) for a in activity_data) / len(activity_data) if activity_data else 0
    avg_sleep = sum(s.get("duration_hours", 0) for s in sleep_data) / len(sleep_data) if sleep_data else 0
...
def _calculate_overall_health_score(user_data: dict) -> int:
    ...
    sleep_data = user_data.get("sleep_data", [])
    if sleep_data:
        avg_sleep = sum(s.get("duration_hours", 0) for s in sleep_data) / len(sleep_data)
...
def _calculate_key_health_metrics(user_data: dict, period_days: int) -> dict:
    ...
    if sleep_data:
        avg_sleep = sum(s.get("duration_hours", 0) for s in sleep_data) / len(sleep_data)
        good_sleep_days = len([s for s in sleep_data if 7 <= s.get("duration_hours", 0) <= 9])
...
def _calculate_data_completeness(user_data: dict, period_days: int) -> dict:
    ...
    completeness["overall"] = round((activity_days + sleep_days + nutrition_days) / (period_days * 3) * 100, 1)
...
def _get_immediate_actions(user_data: dict) -> List[str]:
    ...
    activity_data = user_data.get("activity_data", [])
    if activity_data:
        recent_avg_steps = sum(a.get("steps", 0) for a in activity_data[-7:]) / min(7, len(activity_data))
    ...
    sleep_data = user_data.get("sleep_data", [])
    if sleep_data:
        recent_avg_sleep = sum(s.get("duration_hours", 0) for s in sleep_data[-7:]) / min(7, len(sleep_data))
...
def _get_short_term_goals(user_data: dict) -> List[str]:
    ...
    sleep_data = user_data.get("sleep_data", [])
    if sleep_data:
        good_sleep_ratio = len([s for s in sleep_data if 7 <= s.get("duration_hours", 0) <= 9]) / len(sleep_data)
...
# 体重管理相关辅助函数
def _analyze_weight_trends(weight_history: List[dict], current_weight: float, target_weight: float = None) -> dict:
    """分析体重趋势"""
    ...
    if abs(total_change) < 0.5:
        ...
        trend = "decreasing"
        trend_description = f"体重呈下降趋势，{days_span}天内减少{abs(total_change):.1f}kg"
...
def _generate_weight_management_recommendations(current_weight: float, target_weight: float,
                                              weight_analysis: dict, user_profile) -> List[str]:
    """生成体重管理建议"""
...
def _calculate_weight_timeline(current_weight: float, target_weight: float, weight_analysis: dict) -> dict:
    """计算体重达成时间线"""
    ...
    return {
        ...
        "target_weekly_change": safe_weekly_change if target_weight < current_weight else -safe_weekly_change,

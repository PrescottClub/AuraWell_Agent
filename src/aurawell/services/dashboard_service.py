...
class FamilyDashboardService:
    ...
    async def get_leaderboard(self, metric: str, period: str, family_id: str = None) -> Dict[str, Any]:
        """
        获取家庭排行榜
        ...
        """
        try:
            self.logger.info(f"Getting leaderboard for metric: {metric}, period: {period}")
            
            # 模拟获取排行榜数据
            leaderboard_data = await self._calculate_leaderboard(metric, period, family_id)
    ...
    async def create_challenge(self, family_id: str, challenge_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        创建新的家庭挑战赛
        ...
        try:
            ...
            # 生成挑战赛ID
            challenge_id = f"challenge_{family_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            challenge = {
                ...
                "start_date": challenge_data.get("start_date", datetime.now().date().isoformat()),
    ...
    async def _calculate_leaderboard(self, metric: str, period: str, family_id: str = None) -> Dict[str, Any]:
        """计算排行榜数据"""

...
class DatabaseConfigChecker:
    ...
    def _validate_database_url(self, database_url: str) -> Dict[str, Any]:
        """验证数据库 URL 格式"""
        result = {"valid": True, "errors": [], "recommendations": []}
        ...
        try:
            ...
            if not parsed.scheme:
                ...
                result["errors"].append(
                    "Database URL missing scheme (e.g., sqlite+aiosqlite://)"
                )
                result["recommendations"].append(
                    "Use format: sqlite+aiosqlite:///path/to/db.db"
                )
            ...
            if "sqlite" in parsed.scheme:
                if not parsed.path:
                    ...
                    result["recommendations"].append(
                        "Include database file path: sqlite+aiosqlite:///path/to/db.db"
                    )
                ...
                if "aiosqlite" not in parsed.scheme:
                    result["valid"] = False
                    result["errors"].append(
                        "SQLite URL should use aiosqlite driver for async support"
                    )
                    result["recommendations"].append(
                        "Use sqlite+aiosqlite:// instead of sqlite://"
                    )
        ...
        try:
            ...
            if parsed.path:
                ...
                if not db_file.parent.exists():
                    ...
                    result["recommendations"].append(
                        f"Create directory: mkdir -p {db_file.parent}"
                    )

                # 检查文件权限
                if db_file.exists() and not os.access(db_file, os.R_OK | os.W_OK):
                    result["issues"].append(
                        f"Database file permissions issue: {db_file}"
                    )
                    result["recommendations"].append(
                        f"Fix permissions: chmod 666 {db_file}"
                    )
    ...
    def fix_database_config(self, auto_fix: bool = False) -> Dict[str, Any]:
        ...
        # 自动修复逻辑
        fix_result = {"status": "success", "actions_taken": [], "remaining_issues": []}
            ...
            try:
                ...
                if parsed.path:
                    ...
                    fix_result["actions_taken"].append(
                        f"Created database directory: {db_file.parent}"
                    )
                    ...
                    if not db_file.exists():
                        db_file.touch()
                        fix_result["actions_taken"].append(
                            f"Created database file: {db_file}"
                        )

            except Exception as e:
                fix_result["remaining_issues"].append(
                    f"Failed to create database file: {str(e)}"
                )

...
class UserProfile(BaseModel):
    ...
    activity_level: ActivityLevel = Field(
        ActivityLevel.MODERATELY_ACTIVE, description="Current activity level"
    )
    primary_goal: HealthGoal = Field(
        HealthGoal.GENERAL_WELLNESS, description="Primary health goal"
    )
    secondary_goals: List[HealthGoal] = Field(
        default_factory=list, description="Additional health goals"
    )
    ...
    daily_steps_goal: int = Field(
        10000, ge=1000, le=50000, description="Daily steps target"
    )
    daily_calories_goal: Optional[int] = Field(
        None, ge=1000, le=5000, description="Daily calorie intake target"
    )
    sleep_duration_goal_hours: float = Field(
        8.0, ge=4.0, le=12.0, description="Target sleep duration"
    )
    weekly_exercise_goal_minutes: int = Field(
        150, ge=0, le=1000, description="Weekly exercise target in minutes"
    )

    # Connected platforms
    connected_platforms: List[HealthPlatform] = Field(
        default_factory=list, description="Connected health platforms"
    )
    platform_user_ids: Dict[str, str] = Field(
        default_factory=dict, description="User IDs for each platform"
    )

    # Preferences
    timezone: str = Field("UTC", description="User timezone")
    preferred_units: Literal["metric", "imperial"] = Field(
        "metric", description="Preferred unit system"
    )
...
class UserPreferences(BaseModel):
    ...
    communication_style: Literal["formal", "casual", "motivational", "supportive"] = (
        Field("supportive", description="Preferred AI communication style")
    )
    ...
    # Health focus areas
    focus_areas: List[
        Literal["nutrition", "exercise", "sleep", "stress", "hydration"]
    ] = Field(
        default_factory=lambda: ["exercise", "sleep"],
        description="Primary health focus areas",
    )
    ...
    badge_categories: List[str] = Field(
        default_factory=lambda: ["steps", "workouts", "sleep", "consistency"],
        description="Preferred badge categories",
    )
...
class UserGoalProgress(BaseModel):
    ...
    # Progress tracking
    weekly_progress: List[float] = Field(
        default_factory=list, description="Weekly progress values"
    )

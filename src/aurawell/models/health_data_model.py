...
class UnifiedActivitySummary(BaseModel):
    ...
    active_calories: Optional[float] = Field(
        None, ge=0, description="Active calories burned (kcal)"
    )
    total_calories: Optional[float] = Field(
        None, ge=0, description="Total calories burned (kcal)"
    )
    ...
    data_quality: DataQuality = Field(
        DataQuality.UNKNOWN, description="Data quality indicator"
    )
...
class UnifiedSleepSession(BaseModel):
    ...
    total_duration_seconds: Optional[int] = Field(
        None, ge=0, description="Total sleep duration in seconds"
    )
    ...
    awake_seconds: Optional[int] = Field(
        None, ge=0, description="Awake time during sleep in seconds"
    )
...
class UnifiedHeartRateSample(BaseModel):
    ...
    measurement_type: HeartRateType = Field(
        HeartRateType.UNKNOWN, description="Type of heart rate measurement"
    )
    context: Optional[str] = Field(
        None, description="Measurement context (e.g., 'during_workout', 'at_rest')"
    )
    source_platform: HealthPlatform = Field(..., description="Source health platform")
    data_quality: DataQuality = Field(
        DataQuality.UNKNOWN, description="Data quality indicator"
...
class UnifiedWorkoutSession(BaseModel):
    ...
    calories_burned: Optional[float] = Field(
        None, ge=0, description="Calories burned during workout (kcal)"
    )
    average_heart_rate: Optional[int] = Field(
        None, ge=30, le=220, description="Average heart rate during workout"
    )
    max_heart_rate: Optional[int] = Field(
        None, ge=30, le=220, description="Maximum heart rate during workout"
    )
    distance_meters: Optional[float] = Field(
        None, ge=0, description="Distance covered in meters"
    )
...
class NutritionEntry(BaseModel):
    ...
    protein_grams: Optional[float] = Field(
        None, ge=0, description="Protein content in grams"
    )
    carbs_grams: Optional[float] = Field(
        None, ge=0, description="Carbohydrates content in grams"
    )
    fat_grams: Optional[float] = Field(None, ge=0, description="Fat content in grams")
    fiber_grams: Optional[float] = Field(
        None, ge=0, description="Fiber content in grams"
    )
...
class UnifiedHealthProfile(BaseModel):
    ...
    weight_kg: Optional[float] = Field(
        None, ge=20, le=500, description="Weight in kilograms"
    )

    # Health goals
    daily_steps_goal: Optional[int] = Field(
        None, ge=0, description="Daily steps target"
    )
    daily_calories_goal: Optional[float] = Field(
        None, ge=0, description="Daily calorie burn target"
    )
    sleep_duration_goal_hours: Optional[float] = Field(
        None, ge=4, le=12, description="Sleep duration target in hours"
    )

    # Current averages (last 7 days)
    avg_daily_steps: Optional[int] = Field(
        None, ge=0, description="Average daily steps (7 days)"
    )
    avg_sleep_hours: Optional[float] = Field(
        None, ge=0, description="Average sleep hours (7 days)"
    )
    avg_daily_calories: Optional[float] = Field(
        None, ge=0, description="Average daily calories burned (7 days)"
    )

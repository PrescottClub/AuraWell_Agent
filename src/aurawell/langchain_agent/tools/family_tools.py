    ...
    async def _get_family_context(
        ...
        try:
            ...
            if family_id:
                ...
                # 获取成员信息
                if include_members:
                    members = await self.family_service.get_family_members(
                        family_id, self.user_id
                    )
                    result["members"] = [
                        {
...
class DataComparisonTool(HealthToolAdapter):
    ...
    async def _compare_family_data(
        ...
        try:
            ...
            comparison_data = {
                ...
                "comparison_period": {
                    "start_date": (
                        datetime.now() - timedelta(days=comparison_period_days)
                    ).isoformat(),
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        return {
            "type": "object",
            "properties": {
                "family_id": {"type": "string", "description": "家庭ID"},
                "data_types": {
                    "type": "array",
                    "items": {
                        "type": "string",
...
class GoalSharingTool(HealthToolAdapter):
        ...
        try:
            ...
            return {
                ...
                "data": {
                    ...
                    "active_goals": len(
                        [g for g in mock_goals if g["status"] == "active"]
                    ),
                },
    ...
    async def _track_goal_progress(
        self, family_id: str, goal_id: str
    ) -> Dict[str, Any]:
        """跟踪目标进度"""
        try:
            # 模拟进度跟踪
            ...
            return {"success": False, "error": str(e), "message": "跟踪目标进度失败"}

    async def _create_shared_goal(
        self, family_id: str, goal_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """创建共享目标"""
        try:
            ...
            return {"success": False, "error": str(e), "message": "创建共享目标失败"}

...
class UserRepository(BaseRepository[UserProfileDB]):
    ...
    async def create_user(self, user_profile: UserProfile) -> UserProfileDB:
        ...
        user_data = {
            ...
            + [
                {"type": goal.value, "is_primary": False}
    ...
    async def update_health_goals(
        self, user_id: str, health_goals: List[Dict[str, Any]]
    ) -> Optional[UserProfileDB]:
        """
    ...
    async def get_platform_connection(
        self, user_id: str, platform_name: str
    ) -> Optional[PlatformConnectionDB]:
        """
    ...
    async def update_platform_sync_status(
        self,
        user_id: str,
        platform_name: str,
        sync_status: str,
    ...
    def to_pydantic(self, user_db: UserProfileDB) -> UserProfile:
        ...
        return UserProfile(
            ...
            activity_level=(
                ActivityLevel(user_db.activity_level)
                if user_db.activity_level
                else None
            ),
            ...
            connected_platforms=[
                HealthPlatform(p) for p in user_db.connected_platforms
            ],

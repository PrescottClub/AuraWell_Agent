...
class AchievementRepository(BaseRepository[AchievementProgressDB]):
    ...
    async def save_achievement_progress(
        self,
        user_id: str,
    ...
    async def get_achievement_progress(
        self, user_id: str, achievement_type: str, achievement_level: str
    ) -> Optional[AchievementProgressDB]:
        """
    ...
    async def get_achievement_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        total_stmt = (
            select(func.count())
            .select_from(AchievementProgressDB)
            .where(AchievementProgressDB.user_id == user_id)
        )
        ...
        type_stmt = (
            select(
                ...
                func.sum(cast(AchievementProgressDB.is_unlocked, Integer)).label(
                    "unlocked"
                ),
            )
            .where(AchievementProgressDB.user_id == user_id)
        ...
        achievements_by_type = {
            row.achievement_type: {"total": row.total, "unlocked": row.unlocked or 0}
        ...
        return {
            ...
            "completion_percentage": (
                (unlocked_achievements / total_achievements * 100.0)
    ...
    async def get_leaderboard(
        self, achievement_type: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        if achievement_type:
            ...
            stmt = (
                select(
                    ...
                    func.avg(AchievementProgressDB.progress_percentage).label(
                        "avg_progress"
                    ),
                )
                .where(AchievementProgressDB.achievement_type == achievement_type)
                .group_by(AchievementProgressDB.user_id)
                .order_by(desc("unlocked"), desc("avg_progress"))
                .limit(limit)
            )
        else:
            # Overall leaderboard
            ...
            leaderboard.append(
                {
                    ...
                    "completion_percentage": (
                        (row.unlocked / row.total * 100.0) if row.total > 0 else 0.0
                    ),

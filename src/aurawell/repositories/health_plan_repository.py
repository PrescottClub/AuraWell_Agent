...
class HealthPlanRepository(BaseRepository[HealthPlanDB]):
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.module_repo = BaseRepository[HealthPlanModuleDB](
            session, HealthPlanModuleDB
        )
        self.progress_repo = BaseRepository[HealthPlanProgressDB](
            session, HealthPlanProgressDB
        )
        self.feedback_repo = BaseRepository[HealthPlanFeedbackDB](
            session, HealthPlanFeedbackDB
        )
        self.template_repo = BaseRepository[HealthPlanTemplateDB](
            session, HealthPlanTemplateDB
        )
    ...
    ) -> Optional[HealthPlanDB]:
        """Get health plan by ID with user verification"""
        stmt = (
            select(HealthPlanDB)
            .where(and_(HealthPlanDB.id == plan_id, HealthPlanDB.user_id == user_id))
            .options(
    ...
    async def update_health_plan(
        self, plan_id: str, user_id: str, update_data: Dict[str, Any]
    ) -> Optional[HealthPlanDB]:
        """Update health plan"""
    ...
    async def delete_health_plan(self, plan_id: str, user_id: str) -> bool:
        ...
        stmt = delete(HealthPlanDB).where(
            and_(HealthPlanDB.id == plan_id, HealthPlanDB.user_id == user_id)
        )
        result = await self.session.execute(stmt)
        await self.session.commit()
    ...
    async def create_plan_module(
        self, plan_id: str, module_data: Dict[str, Any]
    ) -> HealthPlanModuleDB:
    ...
    async def get_plan_modules(self, plan_id: str) -> List[HealthPlanModuleDB]:
        ...
        stmt = (
            select(HealthPlanModuleDB)
            .where(HealthPlanModuleDB.plan_id == plan_id)
    ...
    async def update_plan_module(
        self, module_id: str, update_data: Dict[str, Any]
    ) -> Optional[HealthPlanModuleDB]:
        """Update plan module"""
        update_data["updated_at"] = datetime.utcnow()

        stmt = (
            update(HealthPlanModuleDB)
            .where(HealthPlanModuleDB.id == module_id)
            .values(**update_data)
            .returning(HealthPlanModuleDB)
        )
    ...
    async def create_progress_record(
        self, plan_id: str, progress_data: Dict[str, Any]
    ) -> HealthPlanProgressDB:
    ...
    async def get_plan_progress(
        ...
        stmt = select(HealthPlanProgressDB).where(
            HealthPlanProgressDB.plan_id == plan_id
        )
    ...
    async def get_latest_progress(
        self, plan_id: str, module_type: Optional[str] = None
    ) -> Optional[HealthPlanProgressDB]:
    ...
    async def create_feedback(
        self, plan_id: str, feedback_data: Dict[str, Any]
    ) -> HealthPlanFeedbackDB:
    ...
    async def get_plan_feedback(
        ...
        stmt = select(HealthPlanFeedbackDB).where(
            HealthPlanFeedbackDB.plan_id == plan_id
        )
    ...
    async def get_plan_templates(
        ...
        """Get plan templates"""
        stmt = select(HealthPlanTemplateDB).where(
            HealthPlanTemplateDB.is_active == is_active
        )
    ...
    async def increment_template_usage(self, template_id: str) -> None:
        ...
        stmt = (
            update(HealthPlanTemplateDB)
            .where(HealthPlanTemplateDB.id == template_id)
            .values(
    ...
    async def get_plan_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        # Total plans
        total_plans_stmt = select(func.count(HealthPlanDB.id)).where(
